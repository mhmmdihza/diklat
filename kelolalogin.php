...
<div class="row">
  ...
  <div id ="demo" class="col-6 col-s-9">
	<a href="loginform.php" style="text-decoration:none;"><span class="fa-stack">
  ...
  <i class="fa fa-plus fa-stack-1x fa-inverse"></i>
	...
	<?php 	   
	   ...
	   $make_call2 = callAPI('POST', 'http://localhost:8080/login/findall/'.$_SESSION['username'], null);
	   $response2 = json_decode($make_call2, true);
	   
	   

	       echo "<table class='table' id='customers'><tr><th style='width:200px'>Username</th><th>Password</th><th>Sektor</th><th>Hapus/Ubah</th></tr>";
	         for ($x = 0; $x < sizeof($response2); $x++){
	             echo "<tr><td>".$response2[$x]['username']."</td><td>".$response2[$x]['password']."</td><td>".getSektorName($response2[$x]['sektor'])."</td>";?>
	             <td width="90px" align="center">
						<a <?php 
						if($response2[$x]['role']<3){
						    echo 'style="display: none;" ';
						}
						?>href="/hapuslogin.php?id=<?php echo $response2[$x]['username']; ?>" onclick = "if (! confirm('Continue?')) { return false; }"><i class="fa fa-trash-o"></i></a>||
						
						<a  <?php 
						if($response2[$x]['role']<3){
						    echo 'style="display: none;" ';
						}
						?> href="/loginform.php?id=<?php echo $response2[$x]['username'].'"'; ?>
