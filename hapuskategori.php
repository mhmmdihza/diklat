...
if(sizeof($response2)>0){
    ...
    echo "<script>alert('Gagal Menghapus Kategori , Harap hapus jenis barang terlebih dahulu!'); window.location='kategoriperlengkapanoperasional.php?id=1';</script>";
}else{
    echo "test2";
$data_array =  array('id' => $_GET['id'],'password' => '*****');
$make_call = callAPI('POST', 'http://localhost:8080/kategori/delete/'.$_SESSION['username'], json_encode($data_array));
$response = json_decode($make_call, true);
echo "<script>alert('Data berhasil dihapus!'); window.location='kategoriperlengkapanoperasional.php?id=1';</script>";
}
