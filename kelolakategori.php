...
<div class="row">
  ...
  <a href="jenisbarangform.php?idkategori=<?php echo $_GET['id']."&namakategori=".$_GET['nama'];?>" style="text-decoration:none;"><span class="fa-stack">
  ...
  <div class="table-responsive">
	<?php 
	   ...
	   for ($i = 0; $i < sizeof($response2); $i++){
	       ...
	       $make_call3 = callAPI('POST', 'http://localhost:8080/detailBarang/findbyjenis/'.$response2[$i]['id'], null);
	       $response3 = json_decode($make_call3, true);
	       if(sizeof($response3)>0){
	           $dis = '';
	       }
	       echo '<a href="#" onclick="myFunction'.$i.'()" ><i class="fa fa-caret-down"></i></a>';
	       echo '<a href="/hapussubkategori.php?id='.$response2[$i]['id'].'" onclick = "if (! confirm(\'Hapus Jenis Barang '.$response2[$i]['nama'].'?\')) { return false; }">';
	       ...
	       echo '<a href="detailbarangform.php?idmenu='.$_GET['id'].'&idkategori='.$response2[$i]['id'].'&namakategori='.$_GET['nama'].'&jenisbarang='.$response2[$i]['nama'].'" style="text-decoration:none;"><span class="fa-stack">
  <i class="fa fa-circle fa-stack-2x"></i>
  <i class="fa fa-plus fa-stack-1x fa-inverse"></i>
	       ...
	       echo "<table class='table' id='customers'><tr><th style='width:200px'>Jenis</th></tr>";
	         for ($x = 0; $x < sizeof($response3); $x++){
	             echo "<tr><td>".$response3[$x]['id'].'<a href="/hapusdetail.php?id='.
	             $response3[$x]['id'].
