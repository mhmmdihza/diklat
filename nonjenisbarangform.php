...
if(isset($_GET['id'])||!empty($_GET['id'])){
    ...
    $make_callGetData = callAPI('POST', 'http://localhost:8080/subKategori/find', json_encode($data_arrayEdit ));
...
<div class="row">
  ...
  <div id ="demo" class="col-6 col-s-9">
	<br />
	<h3>Kelola Jenis Barang Untuk Kategori <?php echo $_GET['namakategori']?></h3>
	<h4><?php 
	if($isEdit){
	    echo "Ubah Nama Jenis Barang ".$dataEdit['nama'];
	}else{
	    echo "Tambah Jenis Barang Baru";
	}
	?></h4>
	<form action="nonjenisbarangadd.php?idkategori=<?php echo $_GET['idkategori']."&namakategori=".$_GET['namakategori']?>" method="post">
		<?php 
		  if($isEdit){
		      echo '<input type="hidden" name="id" value="'.$dataEdit['id'].'">';
		  }
		  else{
		      echo '<input type="hidden" name="id" value="0">';
		  }
		  echo '<input type="hidden" name="idkategori" value="'.$_GET['idkategori'].'">';
		?>	
		<table>
			<tr>
				<td>Nama Jenis Barang</td>
				<td><input type="text" name="nama" required <?php if($isEdit){
				    echo 'style="background: #dddddd"; value="'.$dataEdit['nama'].'"';
				}?>></td>					
			</tr>			
			</tr>
				<td></td>
