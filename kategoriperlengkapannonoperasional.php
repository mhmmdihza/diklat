...
<div class="row">
  ...
  <h4>Kelola Perlengkapan Non Operasional</h4>
	<a href="nonkategoriform.php" style="text-decoration:none;"><span class="fa-stack">
	...
	<?php 	   
	   ...
	   $response2 = json_decode($make_call2, true);
	         ...
	         for ($x = 0; $x < sizeof($response2); $x++){
	             if($response2[$x]['menu']==1){
	             echo '<li><a href="/kelolanonkategori.php?id='.$response2[$x]['id'].'&nama='.$response2[$x]['nama'].'">'.$response2[$x]['nama'].'</a>';
	             ?>
	             <a href="/hapusnonkategori.php?id=<?php echo $response2[$x]['id']; ?>" onclick = "if (! confirm('Hapus Kategori?')) { return false; }"><i class="fa fa-trash-o"></i></a>
	             <?php 
	             echo '</li>';
	             }
