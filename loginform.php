...
if(isset($_GET['id'])||!empty($_GET['id'])){
    ...
    $make_callGetData = callAPI('POST', 'http://localhost:8080/login/findbyid', json_encode($data_arrayEdit ));
  ...
  <div id ="demo" class="col-6 col-s-9">
	...
	<form action="loginadd.php" method="post">		
		<table>
			<tr>
				<td>Username</td>
				<td><input type="text" name="username" required <?php if($isEdit){
				    echo 'readonly="readonly" style="background: #dddddd"; value="'.$dataEdit['username'].'"';
				}?>></td>					
			</tr>	
			<tr>
				<td>Password</td>
			...
			<tr>
                ...
                <select name="sektor" id="sektor">
                  ...
                  for ($x = 0; $x < sizeof($response2); $x++){
                      echo '<option value="'.$response2[$x]['id'].'">'.$response2[$x]['namaSektor'].'</option>';
                  }?>
