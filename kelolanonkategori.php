...
<div class="row">
  ...
  <a href="nonjenisbarangform.php?idkategori=<?php echo $_GET['id']."&namakategori=".$_GET['nama'];?>" style="text-decoration:none;"><span class="fa-stack">
  <i class="fa fa-circle fa-stack-2x"></i>
  <i class="fa fa-plus fa-stack-1x fa-inverse"></i>
</span> Tambah Jenis Barang Untuk Kategori <?php  echo $_GET['nama']?></a>
<h3>Jenis Barang Kategori <?php  echo $_GET['nama']?></h3>
<h4 ><a style="color:red;">*</a><i style="font-size:12px">Untuk Menghapus Jenis Barang , Diharuskan menghapus detail terlebih dahulu dan tombol hapus akan muncul setelah semua detail di jenis barang tersebut sudah kosong</i></h4>
  <div class="table-responsive">
	<?php 
	   ...
	   for ($i = 0; $i < sizeof($response2); $i++){
	       ...
	       $make_call3 = callAPI('POST', 'http://localhost:8080/nondetailBarang/findbyjenis/'.$response2[$i]['id'], null);
	       $response3 = json_decode($make_call3, true);
	       if(sizeof($response3)>0){
	           $dis = '';
	       }
	       echo '<a href="#" onclick="myFunction'.$i.'()" ><i class="fa fa-caret-down"></i></a>';
	       echo '<a href="/nonhapussubkategori.php?id='.$response2[$i]['id'].'" onclick = "if (! confirm(\'Hapus Jenis Barang '.$response2[$i]['nama'].'?\')) { return false; }">';
	       if(sizeof($response3)<1){
	           echo '<i class="fa fa-trash-o"></i></a>';
	       }else {
	           echo '</a>';
	       }
	       echo '<div id="myDIV'.$i.'" style="display: none;">';
	       echo '<a href="nondetailbarangform.php?idmenu='.$_GET['id'].'&idkategori='.$response2[$i]['id'].'&namakategori='.$_GET['nama'].'&jenisbarang='.$response2[$i]['nama'].'" style="text-decoration:none;"><span class="fa-stack">
  <i class="fa fa-circle fa-stack-2x"></i>
  <i class="fa fa-plus fa-stack-1x fa-inverse"></i>
</span> Tambah Detail Barang '.$response2[$i]['nama'].'</a>';
	       echo "<table class='table' id='customers'><tr><th style='width:200px'>Jenis</th></tr>";
	         for ($x = 0; $x < sizeof($response3); $x++){
	             echo "<tr><td>".$response3[$x]['id'].'<a href="/nonhapusdetail.php?id='.
	             $response3[$x]['id'].
	             '" onclick = "if (! confirm(\'Hapus detail Barang '.$response3[$x]['id'].'?\')) { return false; }"><i class="fa fa-trash-o"></i></a>'."</td>";
	             echo "</tr>";
	       };echo "</table>"; 
    echo '</div>';
	       echo "</p>";
