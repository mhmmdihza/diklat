...
$data_array =  array('idJenis' => $id, 'reason' => $keterangan,'baik'=>$baik,'rusak'=>$rusak,'status'=>'Waiting','menu' =>$_GET['menu'],'jenisDest' =>0,'sektorDest'=> $sektor);
 $make_call = callAPI('POST', 'http://localhost:8080/nonolahstok/save/1/'.$_SESSION['username'], json_encode($data_array));
 $response = json_decode($make_call, true);
 
 if($response){
 echo "<script>alert('Data berhasil ditambahkan!'); window.location='/nonkategori.php?id=".$_GET['returnid']."&sektor=".$_GET['returnsektor']."';</script>";
 }else{
